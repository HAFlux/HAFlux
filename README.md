...
Деплой — **pull-only**: образ `ghcr.io/haflux/api` тянется готовым, ничего не
собирается на сервере. Чтобы обновить уже работающую инсталляцию:

```bash
cd /opt/haflux
# 1. ОБЯЗАТЕЛЬНО: бэкап БД перед апгрейдом
docker exec haflux-db-1 pg_dump -U haflux haflux | gzip > backup-$(date +%F-%H%M).sql.gz
# 2. зафиксировать конкретную версию (не latest — он уезжает с каждым релизом)
sed -i 's/^HAFLUX_API_TAG=.*/HAFLUX_API_TAG=v0.5.2/' .env
# 3. подтянуть образ и пересоздать только api
docker compose --env-file .env pull api
docker compose --env-file .env up -d api
# 4. проверить, что api поднялся и данные на месте
curl -fsS http://127.0.0.1:3000/health
```

> При старте контейнер сам приводит схему БД к актуальной (`prisma db push`).
> Откат: верни прежний `HAFLUX_API_TAG`, `up -d api`; при необходимости —
> восстанови дамп. Свежий релиз меняет место панели — см. ниже.

> **Панель переезжает на свой порт.** Начиная с v0.5, панель слушает только
> `HAFLUX_WEB_PORT` (по умолчанию `8080`, https self-signed), а `:80`/`:443`
> отдаются под proxy host'ы. На инсталляциях, обновлённых со старых версий,
> панель остаётся на `:80` до **первого** «Применить» в кластере — после
> рендера нового конфига она уедет на `:8080`, а неизвестный домен на
> `:80`/`:443` начнёт получать 404 вместо панели. Не потеряй адрес из закладок.

## Что нового в v0.5

- **Мультинодовость** — кластеры из нескольких HAProxy-нод (LOCAL + удалённые по
  SSH), доставка конфига на каждую ноду, health-check, ручной promote,
  auto-failover ролей для `ACTIVE_PASSIVE` (детали ниже).
- **Гео-фильтрация в группах доступа** — блок-лист стран (403 для перечисленных)
  **и** алло-лист (пускать ТОЛЬКО из выбранных стран); диапазоны IP берутся с
  ipdeny.com и обновляются раз в неделю. В пикере — флаги стран.
- **IP denylist** в группах доступа (проверяется раньше allowlist/auth).
- **Проверка апстрима** из модалки хоста (HTTP/HTTPS/TCP), фоновый health-probe.
- **Brute-force throttle** на логин (Redis или in-memory): 5 фейлов/15 мин на
  email, 20 на IP.
- **Валидация конфига** `haproxy -c` перед reload, с откатом при ошибке.
- **Кастомный log-format** на кластер.
- Удалены HSTS и HTTP/3; ребрендинг HPM → HAFlux.

> **Безопасность basic-auth.** Пароли пользователей групп доступа HAProxy
> требует в открытом виде в конфиге (`insecure-password`), поэтому они
> рендерятся в `haproxy.cfg` (mode 0640) и доставляются на ноды. Доступ к
> файловой системе ноды = раскрытие этих паролей. Не используй для них
> ценные/переиспользуемые пароли.

 ...
 *  - Per-host логика: hdr(host) на :80 + ssl_fc_sni на :443.
...
<div class="meta"><span>HAProxy Packet Manager</span><span>code: ${code}</span></div>
  ...
  private renderCfg(
    ...
    const out: string[] = [];
    out.push(`# Auto-generated by HPM. Do not edit manually.
...
`);
    ...
    if (hasCerts) {
      const sslHosts = httpHosts.filter((h) => h.ssl);
      const anyH2 = sslHosts.some((h) => h.http2);
      const anyH3 = sslHosts.some((h) => h.http3);
      ...
      for (const h of sslHosts) {
        if (h.domain.startsWith('*.')) {
          out.push(`  acl ssl_${h.id} ssl_fc_sni_end -i .${h.domain.slice(2)}`);
        } else {
          out.push(`  acl ssl_${h.id} ssl_fc_sni -i ${h.domain}`);
        }
      }
      out.push('');
      ...
      for (const h of sslHosts) {
        if (h.hsts) {
          out.push(
            `  http-response set-header Strict-Transport-Security "max-age=31536000; includeSubDomains" if ssl_${h.id}`,
          );
        }
      }
      ...
      for (const h of sslHosts) {
        if (h.blockExploits) {
          out.push(
            `  http-request deny if ssl_${h.id} { path_beg /.git /.env /.svn /.aws /.docker }`,
          );
        }
      }
      ...
      for (const h of sslHosts) {
        ...
        if (ipGids.length > 0) {
          const negations = ipGids.map((gid) => `!${this.aclSrcName(gid)}`).join(' ');
          out.push(`  http-request deny deny_status 403 if ssl_${h.id} ${negations}`);
        }
        const needsAuth = h.accessGroupIds.some((gid) => groupsById.get(gid)?.hasAuth);
        if (needsAuth) {
          const ulName = this.hostMergedUlName(h.id);
          const ackName = `auth_ok_${h.id}`;
          out.push(`  acl ${ackName} http_auth(${ulName})`);
          out.push(`  http-request auth realm "HPM" if ssl_${h.id} !${ackName}`);
        }
      }

      // Misdirected должен идти ДО use_backend (HAProxy 3 ругается на
      // 'http-request' после 'use_backend').
      out.push(
        '  http-request return status 421 content-type text/plain string "Misdirected" if !{ ssl_fc_sni -m found }',
      );

      for (const h of sslHosts) {
        out.push(`  use_backend be_proxy_${h.id} if ssl_${h.id}`);
      }
      out.push('');
    }
